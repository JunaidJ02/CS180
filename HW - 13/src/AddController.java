...
public final class AddController {
    ...
    private void getAddButtonSemantics() {
        if (inventoryModel.searchBySku(addView.getSkuTextField().getText()).isPresent()) {
        ...
        } else if (!addView.getWholesalePriceTextField().getText().matches("^[0-9.]+$")) {
            ...
            inventoryModel.add(new Product(addView.getSkuTextField().getText(), addView.getNameTextField().getText(),
                    Double.parseDouble(addView.getWholesalePriceTextField().getText()), Double.parseDouble(addView.getRetailPriceTextField().getText()),
